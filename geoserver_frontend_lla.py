...
class Ship:
    def __init__(self, environment, ship_id: int, lat: float = 0.0, lon: float = 0.0, speed=5, screen_width: int = 600, screen_height: int = 400,
        ...
        self.prev_lla_in_funnel = False  # Track previous in_funnel state for reward shaping
    ...
    def validate_and_fire(self, ship, target):
        ...
        if current_time - ship.last_fire_time >= ship.reload_delay:
            ...
            if current_time - ship.target_lock_time >= ship.target_delay:
                ...
                # Additional LLA check: only fire if in funnel
                if can_fire:
                    in_funnel = self._check_lla_in_funnel(ship, target)
                    if in_funnel:
                        return self.fire_torpedo(ship, target)
                    else:
                        # Small penalty for attempting to fire outside funnel
                        if ship.ship_type == 'attacker_ship':
                            self.env.reward -= 1  # Penalty for attacker firing outside funnel
                        else:
                            self.env.reward += 0.5  # Small reward for defender prevented from firing (good for attacker)
        return False

    def _check_lla_in_funnel(self, ship, target):
        """Check if the ship is in the LLA funnel for targeting."""
        # Helicopter ships don't need LLA checks (they don't fire torpedoes)
        if ship.ship_type == 'def_heli':
            return False  # Helicopters can't fire torpedoes
        
        # Determine the appropriate LLA data key
        if ship.ship_type == 'attacker_ship':
            lla_key = 'attacker'
        else:
            # Find defender index
            for i, defender in enumerate(self.env.defender_ships):
                if defender == ship:
                    lla_key = f'defender_{i}'
                    break
            else:
                # Ship not found in defender list - this shouldn't happen
                return False  # Block firing if ship not found
        
        # Check LLA data
        lla_data = self.env.lla_data.get(lla_key)
        if lla_data:
            return lla_data['in_funnel']
        
        # No LLA data available - block firing for safety
        return False
...
class NavalShipEnv(gym.Env):
    ...
    def _get_obs(self):
        # Return the positions of all ships + LLA features for shooting agents
        observation = []
        ...
        # Add LLA features for shooting agents
        # First, add attacker LLA features
        if 'attacker' in self.lla_data:
            observation.extend(self.lla_data['attacker']['features'])
        else:
            # Default values if LLA not computed yet
            observation.extend([0.0, 0.0, 0.0])
        
        # Then, add defender LLA features (skip helicopter)
        for i, defender in enumerate(self.defender_ships):
            if defender.ship_type == 'def_heli':
                continue  # Skip helicopter (no weapons)
            
            defender_key = f'defender_{i}'
            if defender_key in self.lla_data:
                observation.extend(self.lla_data[defender_key]['features'])
            else:
                # Default values if LLA not computed yet
                observation.extend([0.0, 0.0, 0.0])
    ...
    # 2. Compute LLA for all shooting agents
    ...
    # 4. Calculate reward and handle torpedo firing logic (includes LLA reward shaping)
    ...
    def _compute_lla_data(self):
        """Compute LLA data for all shooting agents (attacker and defenders)."""
        self.lla_data = {}
        
        # Compute LLA for attacker targeting HVU
        ...
        attacker_features = compute_lla_features(
            ...
            self.attacker_ship.prev_lla_in_funnel
        )
        ...
        # Compute LLA for each defender targeting attacker
        for i, defender in enumerate(self.defender_ships):
...
#         return
    ...
    def _calculate_lla_reward_shaping(self):
        ...
        # Check attacker LLA reward shaping
        if 'attacker' in self.lla_data:
            current_in_funnel = self.lla_data['attacker']['in_funnel']
            # Reward for entering funnel
            if current_in_funnel and not self.attacker_ship.prev_lla_in_funnel:
                reward += 5  # Small reward for entering funnel
            
            # Update previous state
            self.attacker_ship.prev_lla_in_funnel = current_in_funnel
        
        # Check defender LLA reward shaping
        for i, defender in enumerate(self.defender_ships):
            if defender.ship_type == 'def_heli':
                continue  # Skip helicopter
            ...
            defender_key = f'defender_{i}'
            if defender_key in self.lla_data:
                current_in_funnel = self.lla_data[defender_key]['in_funnel']
                # Reward for entering funnel (from defender's perspective, this is good for defense)
                if current_in_funnel and not defender.prev_lla_in_funnel:
                    reward -= 2  # Small penalty when defender enters attack funnel (bad for attacker)
                
                # Update previous state
                defender.prev_lla_in_funnel = current_in_funnel
