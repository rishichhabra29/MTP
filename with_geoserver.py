...
# LLA (Limited Lines of Approach) Helper Functions
def compute_lla(shooter_pos, target_pos, range_radius):
    """
    Compute Limited Lines of Approach (LLA) for a shooter targeting a danger zone circle.
    
    Args:
        shooter_pos: (x, y) position of the shooter
        target_pos: (x, y) position of the target (center of danger zone)
        range_radius: radius of the danger zone circle
    
    Returns:
        tuple: (theta0, alpha) where theta0 is bearing to target, alpha is half-aperture
        None: if shooter is inside the range circle (can fire in any direction)
    """
    dx = target_pos[0] - shooter_pos[0]
    dy = target_pos[1] - shooter_pos[1]
    d = hypot(dx, dy)
    
    # If shooter is inside the range circle, it can fire in any direction
    if d <= range_radius:
        return None
    
    # Calculate bearing to target and half-aperture angle
    theta0 = atan2(dy, dx)
    alpha = acos(range_radius / d)
    
    return theta0, alpha

def compute_lla_features(shooter_pos, target_pos, range_radius, shooter_heading, prev_in_funnel=False):
    """
    Compute LLA observation features for a shooter with hysteresis to prevent blinking.
    
    Args:
        shooter_pos: (x, y) position of the shooter
        target_pos: (x, y) position of the target
        range_radius: radius of the danger zone
        shooter_heading: current heading of the shooter in radians
        prev_in_funnel: previous in_funnel state for hysteresis
    
    Returns:
        tuple: (normalized_half_aperture, normalized_bearing_error, in_funnel)
    """
    lla_result = compute_lla(shooter_pos, target_pos, range_radius)
    
    if lla_result is None:
        # Inside range circle - can fire in any direction
        return 0.0, 0.0, True
    
    theta0, alpha = lla_result
    
    # Normalized half-aperture
    normalized_half_aperture = alpha / pi
    
    # Normalized bearing error - using actual current heading vs optimal bearing
    bearing_error = (shooter_heading - theta0) / alpha if alpha > 0 else 0
    normalized_bearing_error = max(-1.0, min(1.0, bearing_error))  # Clip to [-1, 1]
    
    # In funnel check with hysteresis to prevent blinking
    abs_bearing_error = abs(bearing_error)
    
    if prev_in_funnel:
        # If previously in funnel, use higher threshold to exit (hysteresis)
        in_funnel = abs_bearing_error <= 1.4  # More forgiving to stay in
    else:
        # If previously out of funnel, use lower threshold to enter
        in_funnel = abs_bearing_error <= 1.0  # Stricter to enter
    
    return normalized_half_aperture, normalized_bearing_error, in_funnel
...
class Ship:
    def __init__(self, environment, ship_id: int, lat: float = 0.0, lon: float = 0.0, speed=5, screen_width: int = 600, screen_height: int = 400,
        ...
        # LLA-related attributes
        self.heading = 0.0  # Current heading in radians
        # prev_lla_in_funnel removed - no longer needed
    ...
    # Update x/y projection from lat/lon
        ...
        # Update heading for LLA calculations (convert degrees to radians)
...
class FireMechanism:
    ...
    def _handle_defender_firing(self, defender):
        ...
        penalty = 0
        # Skip helicopter ships (they don't fire torpedoes)
        if defender.ship_type == 'def_heli':
            return penalty
    ...
    def validate_and_fire(self, ship, target):
        ...
        if current_time - ship.last_fire_time >= ship.reload_delay:
            ...
            if current_time - ship.target_lock_time >= ship.target_delay:
                ...
                # LLA firing control removed - ships can fire based on LOS and range only
                if can_fire:
                    return self.fire_torpedo(ship, target)
        return False

    # LLA firing control method removed - no longer needed
...
class DecoyMissileManager:
    def __init__(self, env):
        self.env = env
        self.neutralized_torpedoes = set()  # Track which torpedoes have been neutralized
        self.decoy_effectiveness = {}  # Track which decoy neutralized which torpedo
    
    def reset_neutralization_tracking(self):
        """Reset neutralization tracking for new episode."""
        self.neutralized_torpedoes.clear()
        self.decoy_effectiveness.clear()
    
    def get_decoy_effectiveness_stats(self):
        """Get statistics about decoy effectiveness."""
        return {
            'total_torpedoes_neutralized': len(self.neutralized_torpedoes),
            'decoy_effectiveness': self.decoy_effectiveness.copy(),
            'neutralized_torpedo_ids': list(self.neutralized_torpedoes)
        }
    ...
    def fire_missile(self, ship, target):
        if ship.decoyM_count > 0:
            ...
            missile = {
                ...
                'target': target,
                'target_hit': False,
                'neutralized_torpedo': None  # Track which torpedo this decoy neutralizes
            }
    ...
    def update_decoy_missile(self):
        for ship in self.env.defender_ships:
            ...
            for missile in ship.decoy_missile:
                ...
                # Check for hit (decoy hitting torpedo)
                ...
                if distance <= ship.decoyM_blast_range and not missile.get('target_hit', False):
                    # Check if this torpedo has already been neutralized by another decoy
                    if target.id not in self.neutralized_torpedoes:
                        missile['target_hit'] = True
                        missile['neutralized_torpedo'] = target  # Track which torpedo was neutralized
                        
                        # Mark this torpedo as neutralized
                        self.neutralized_torpedoes.add(target.id)
                        self.decoy_effectiveness[missile['id']] = target.id
                        ...
                        if target in target.source.torpedoes:
                            ...
                            print(f"Decoy missile {missile['id']} neutralized torpedo {target.id}")
                    else:
                        # This torpedo was already neutralized by another decoy
                        missile['target_hit'] = True
                        print(f"Decoy missile {missile['id']} hit already neutralized torpedo {target.id}")

                # Check for torpedo hitting decoy (bidirectional collision)
                self._check_torpedo_hitting_decoy(missile, ship)
    ...
    def _check_torpedo_hitting_decoy(self, missile, ship):
        """Check if any torpedo is hitting this decoy missile (bidirectional collision)."""
        # Check all torpedoes from attacker
        for torpedo in self.env.attacker_ship.torpedoes:
            if torpedo.id in self.neutralized_torpedoes:
                continue  # Skip already neutralized torpedoes
                
            # Get torpedo pixel position
            torpedo.update_pixel_position()
            torpedo_pos = np.array([torpedo.x, torpedo.y])
            missile_pos = np.array([missile['x'], missile['y']])
            
            # Calculate distance between torpedo and decoy
            distance = np.linalg.norm(torpedo_pos - missile_pos)
            
            # If torpedo hits decoy within blast range
            if distance <= ship.decoyM_blast_range and not missile.get('target_hit', False):
                missile['target_hit'] = True
                missile['neutralized_torpedo'] = torpedo  # Track which torpedo was neutralized
                
                # Mark this torpedo as neutralized
                self.neutralized_torpedoes.add(torpedo.id)
                self.decoy_effectiveness[missile['id']] = torpedo.id
                
                # Remove the torpedo from the game when hit by decoy missile
                if torpedo in torpedo.source.torpedoes:
                    torpedo.source.torpedoes.remove(torpedo)
                    print(f"Decoy missile {missile['id']} neutralized torpedo {torpedo.id} (bidirectional hit)")
                break  # Only neutralize one torpedo per decoy
...
class UIManager:
    ...
    def get_user_input_screen(self):
        ...
        show_firing_ranges = True  # Default to showing firing range circles
        error_message = ""
        done = False
        ...
        while not done:
            ...
            self._render_firing_range_checkbox(show_firing_ranges)
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    checkbox_checked, show_firing_ranges, input_boxes, colors = self._handle_mouse_click(
                        event.pos, checkbox_checked, show_firing_ranges, input_boxes
                    )
        ...
        return self._finalize_user_inputs(user_inputs, checkbox_checked, show_firing_ranges)
    ...
    def _render_firing_range_checkbox(self, checked):
        self.firing_range_checkbox_rect = pygame.Rect(100, 310, 20, 20)
        pygame.draw.rect(self.env.screen, (0, 0, 0), self.firing_range_checkbox_rect, 2)  # Draw outer border

        if checked:
            pygame.draw.rect(self.env.screen, (0, 0, 0), self.firing_range_checkbox_rect.inflate(-8, -8))  # Fill if checked

        # Label beside checkbox
        label = self.label_font.render("Show Firing Range Circles:", True, (0, 0, 0))
        self.env.screen.blit(label, (130, 310))
...
#             self.env.screen.blit(self.label_font.render(formation.capitalize(), True, (0, 0, 0)), (rect.x + 30, rect.y))
    ...
    def _render_inputs(self, inputs, boxes, colors, custom_enabled):
        ...
        submit_rect = pygame.Rect(100, 520 if custom_enabled else 370, 180, 40)
    ...
    def _handle_mouse_click(self, pos, checkbox_state, show_firing_ranges, input_boxes):
        if self.checkbox_rect.collidepoint(pos):
            checkbox_state = not checkbox_state
        
        if hasattr(self, 'firing_range_checkbox_rect') and self.firing_range_checkbox_rect.collidepoint(pos):
            show_firing_ranges = not show_firing_ranges
        ...
        return checkbox_state, show_firing_ranges, input_boxes, colors
    ...
    def _finalize_user_inputs(self, inputs, custom, show_firing_ranges):
        ...
        #pygame.display.quit()
        #pygame.display.init()
        return (num_defenders, custom, num_sonar, num_heli, num_decoy, num_default, 
                formation, [lat, lon], zoom, [base_lat, base_lon], moving_formation, 
                show_firing_ranges)
...
class Renderer:
    ...
    def _render_ship(self, ship, color=(255, 255, 255), base_rect_size=15, line_thickness=1):
        ...
        if ship.ship_health > 0:
            ...
            # Draw firing range (circle) around the ship - only if option is enabled
            if ship.firing_range > 0 and self.env.show_firing_ranges:
    ...
    def _render_helicopter(self, latlon):
        ...
        if self.env.helicopManager.helicopter_lat is not None:
            ...
            # Draw detection range circle - only if option is enabled
            if self.env.show_firing_ranges:
    ...
    def _render_lla_funnels(self):
        """
        Render Limited Lines of Approach (LLA) tangent lines.
        """
        if not hasattr(self.env, 'lla_data') or not self.env.lla_data:
            return
        
        # Color scheme for different ship types
        ATTACKER_GOOD_COLOR = (50, 255, 50)     # Green when attacker in funnel
        ATTACKER_BAD_COLOR = (255, 50, 50)      # Red when attacker out of funnel
        DEFENDER_GOOD_COLOR = (50, 255, 50)     # Green when defender in funnel  
        DEFENDER_BAD_COLOR = (50, 100, 255)     # Blue when defender out of funnel
        
        # Render attacker LLA (targeting HVU)
        if 'attacker' in self.env.lla_data:
            attacker_pos = self.env.attacker_ship.get_position()
            hvu_pos = self.env.hvu.get_position()
            in_funnel = self.env.lla_data['attacker']['in_funnel']
            
            # Attacker lines: RED → GREEN
            scaled_range = self._get_range_scale(self.env.attacker_range)
            line_color = ATTACKER_GOOD_COLOR if in_funnel else ATTACKER_BAD_COLOR
            self._draw_simple_lla_tangents(attacker_pos, hvu_pos, scaled_range, line_color)
        
        # Render defender LLAs (targeting attacker)
        attacker_pos = self.env.attacker_ship.get_position()
        for i, defender in enumerate(self.env.defender_ships):
            if defender.ship_type == 'def_heli':
                continue  # Skip helicopter
                
            defender_key = f'defender_{i}'
            if defender_key in self.env.lla_data:
                defender_pos = defender.get_position()
                in_funnel = self.env.lla_data[defender_key]['in_funnel']
                
                # Defender lines: BLUE → GREEN
                scaled_range = self._get_range_scale(defender.firing_range)
                line_color = DEFENDER_GOOD_COLOR if in_funnel else DEFENDER_BAD_COLOR
                self._draw_simple_lla_tangents(defender_pos, attacker_pos, scaled_range, line_color)

    def _get_range_scale(self, base_range):
        """Get scaled range for visualization - keeps LLA lines visible at all zoom levels."""
        # More conservative scaling to prevent ranges from becoming too large
        if self.env.zoom <= 3:
            # Very zoomed out - limit scaling to prevent huge ranges
            scale = min(1.5, 6.0 / self.env.zoom)
        elif self.env.zoom < 6:
            # Moderately zoomed out - normal scaling
            scale = 6.0 / self.env.zoom
        else:
            # Zoomed in - scale down
            scale = 6.0 / self.env.zoom
        
        # Balanced min/max to keep LLA geometry visible
        scaled_range = max(40, min(200, int(base_range * scale)))
        return scaled_range

    def _draw_simple_lla_tangents(self, shooter_pos, target_pos, target_radius, line_color):
        """Draw LLA funnels (wedge-shaped areas) for better tactical visualization."""
        lla_result = compute_lla(shooter_pos, target_pos, target_radius)
        
        if lla_result is None:
            # Inside range - draw a small funnel instead of circle
            # Create a small wedge to show firing capability
            theta0 = math.atan2(target_pos[1] - shooter_pos[1], target_pos[0] - shooter_pos[0])
            alpha = math.pi / 6  # 30 degree wedge
            
            # Calculate small funnel points
            left_angle = theta0 - alpha
            right_angle = theta0 + alpha
            
            # Small funnel length (20 pixels)
            funnel_length = 20
            left_end_point = (
                shooter_pos[0] + funnel_length * math.cos(left_angle),
                shooter_pos[1] + funnel_length * math.sin(left_angle)
            )
            right_end_point = (
                shooter_pos[0] + funnel_length * math.cos(right_angle),
                shooter_pos[1] + funnel_length * math.sin(right_angle)
            )
            
            # Create small funnel shape
            funnel_points = [
                (int(shooter_pos[0]), int(shooter_pos[1])),  # Shooter position (apex)
                (int(left_end_point[0]), int(left_end_point[1])),   # Left boundary
                (int(right_end_point[0]), int(right_end_point[1]))  # Right boundary
            ]
            
            # Create semi-transparent fill color
            fill_color = (*line_color, 76)  # RGBA with alpha=76 (30% of 255)
            
            # Create a surface for the semi-transparent funnel
            funnel_surface = pygame.Surface((self.env.width, self.env.height), pygame.SRCALPHA)
            pygame.draw.polygon(funnel_surface, fill_color, funnel_points)
            
            # Blit the semi-transparent funnel to the main screen
            self.env.screen.blit(funnel_surface, (0, 0))
            
            # Draw boundary lines for clarity
            line_width = 2
            pygame.draw.line(self.env.screen, line_color, 
                            (int(shooter_pos[0]), int(shooter_pos[1])), 
                            (int(left_end_point[0]), int(left_end_point[1])), line_width)
            pygame.draw.line(self.env.screen, line_color, 
                            (int(shooter_pos[0]), int(shooter_pos[1])), 
                            (int(right_end_point[0]), int(right_end_point[1])), line_width)
            return
        
        theta0, alpha = lla_result
        
        # Calculate tangent directions (angles)
        left_angle = theta0 - alpha
        right_angle = theta0 + alpha
        
        # Define maximum line length scaled with zoom - maintains proportions at all zoom levels
        base_line_length = 80  # Base line length in pixels
        zoom_scale_factor = 6.0 / max(1, self.env.zoom)  # Same scaling logic as range scaling
        max_line_length = max(40, min(120, int(base_line_length * zoom_scale_factor)))
        
        # Calculate limited end points based on direction and max length
        left_end_point = (
            shooter_pos[0] + max_line_length * math.cos(left_angle),
            shooter_pos[1] + max_line_length * math.sin(left_angle)
        )
        right_end_point = (
            shooter_pos[0] + max_line_length * math.cos(right_angle),
            shooter_pos[1] + max_line_length * math.sin(right_angle)
        )
        
        # Create funnel (wedge) shape points
        funnel_points = [
            (int(shooter_pos[0]), int(shooter_pos[1])),  # Shooter position (apex)
            (int(left_end_point[0]), int(left_end_point[1])),   # Left boundary
            (int(right_end_point[0]), int(right_end_point[1]))  # Right boundary
        ...
        # Draw small markers at end points
        pygame.draw.circle(self.env.screen, line_color, 
                         (int(left_end_point[0]), int(left_end_point[1])), 3)
        pygame.draw.circle(self.env.screen, line_color, 
                         (int(right_end_point[0]), int(right_end_point[1])), 3)
...
class NavalShipEnv(gym.Env):
    ...
    def __init__(
        ...
        # Initialize Pygame and basic display only if render mode is enabled
        if self.render_mode:
            pygame.init()
            ...
            # Set dimensions but don't initialize pygame display
            self.width, self.height = screen_width, screen_height
            self.font = None
        ...
        # Get User Input for environment configuration only if render mode is enabled
        if self.render_mode:
            (
                self.num_defenders,
                self.custmise_def,
                ...
                self.show_firing_ranges
            ) = UIManager(self).get_user_input_screen()
            # 🧹 Clear leftover UI screen after input
            self.screen.fill((0, 0, 0))
            pygame.display.flip()
        else:
            # Use default values when render mode is off
            self.custmise_def = False
            self.def_formation = def_default_formation
            self.show_firing_ranges = True  # Default to showing firing ranges



        # Define Observation Space (Attacker + HVU + all Defenders + LLA features)
        total_ships = self.num_defenders + 2
        # Count shooting agents (attacker + defenders except helicopter)
        shooting_agents = 1 + (self.num_defenders - self.num_def_with_helicopter)  # attacker + non-heli defenders
        
        # Position features: 2 * total_ships (x, y for each ship)
        # LLA features: 3 * shooting_agents (half_aperture, bearing_error, in_funnel for each shooter)
        total_obs_size = 2 * total_ships + 3 * shooting_agents
        
        # Create bounds for observation space
        position_bounds_low = np.zeros(2 * total_ships, dtype=np.float32)
        position_bounds_high = np.array([self.width, self.height] * total_ships, dtype=np.float32)
        
        # LLA feature bounds: [0, 1] for half_aperture, [-1, 1] for bearing_error, [0, 1] for in_funnel
        lla_bounds_low = np.tile([0.0, -1.0, 0.0], shooting_agents).astype(np.float32)
        lla_bounds_high = np.tile([1.0, 1.0, 1.0], shooting_agents).astype(np.float32)
        
        self.observation_space = spaces.Box(
            low=np.concatenate([position_bounds_low, lla_bounds_low]),
            high=np.concatenate([position_bounds_high, lla_bounds_high]),
            dtype=np.float32
        )
    ...
    def reset(self):
        ...
        # Initialize Map Renderer and background image only if render mode is enabled
        if self.render_mode:
        ...
        # Reset decoy neutralization tracking
        self.DecoyMissileManager.reset_neutralization_tracking()
        ...
        # Initialize LLA-related attributes
        self.lla_data = {}  # Store LLA calculations for all shooting agents
        self.attacker_range = self.attacker_ship.firing_range
        # Defender ranges (all defenders should have same firing range for now, but we could make it more flexible)
        self.defender_range = self.defender_ships[0].firing_range if self.defender_ships else 100

        # Return initial observation
        return self._get_obs()

    def _reload_map_image(self):
        # Only reload map image if render mode is enabled
    ...
    def _get_obs(self):
        # Return the positions of all ships only (LLA features removed)
        observation = []
        
        # Add position features for all ships
        for ship in self.ships:
            observation.extend(ship.get_position())
        
        # LLA features removed from observations to prevent interference with formations and firing
    ...
    # 2. Compute LLA data for visual funnel display only
        self._compute_lla_data()

    # 3. Update torpedoes and decoys
        self.firemechanism._update_torpedo_position()
        self.DecoyMissileManager.update_decoy_missile()

    # 4. Calculate reward and handle torpedo firing logic
        self.reward += self._calculate_reward()

    # ✅ 5. Now check for ship collisions AFTER firing happens
    ...
    def _compute_lla_data(self):
        """Compute LLA data for visual funnel display only (not for firing control)."""
        self.lla_data = {}
        
        # Compute LLA for attacker targeting HVU (visual only)
        attacker_pos = self.attacker_ship.get_position()
        hvu_pos = self.hvu.get_position()
        attacker_features = compute_lla_features(
            attacker_pos, hvu_pos, self.attacker_range, self.attacker_ship.heading,
            False  # prev_lla_in_funnel removed
        )
        self.lla_data['attacker'] = {
            'features': attacker_features,
            'in_funnel': attacker_features[2]
        }
        
        # Compute LLA for each defender targeting attacker (visual only)
        for i, defender in enumerate(self.defender_ships):
            # Skip helicopter (no weapons)
            if defender.ship_type == 'def_heli':
                continue
                
            defender_pos = defender.get_position()
            defender_features = compute_lla_features(
                defender_pos, attacker_pos, defender.firing_range, defender.heading,
                False  # prev_lla_in_funnel removed
            )
            self.lla_data[f'defender_{i}'] = {
                'features': defender_features,
                'in_funnel': defender_features[2]
            }
...
#         return
    ...
    def _calculate_reward(self):
        ...
        # LLA reward shaping - REMOVED to prevent interference with formations
        # reward += self._calculate_lla_reward_shaping()

        return reward

    def _calculate_lla_reward_shaping(self):
        """Calculate reward shaping based on LLA funnel entry and firing outside funnel."""
        # REMOVED: LLA reward shaping that interferes with formations
        # LLA should only control weapon firing, not movement rewards
        return 0
    ...
    def render(self, mode='human'):
        # Only render if render mode is enabled
        if not self.render_mode:
            return
    ...
    # Draw ships and simulation elements
        ...
        # Render LLA funnels
        self.renderer._render_lla_funnels()
...
if __name__ == "__main__":
    ...
    # Example 1: Environment with rendering enabled (default behavior)
    print("Running environment with rendering enabled...")
    env = NavalShipEnv(
        screen_width=1000,
        screen_height=600,
        ...
        helicop_range=150,
        helicop_speed=130,
        render_mode=True # Enable rendering - pygame window will appear
    )
    ...
    running = True
    while running:
        if env.render_mode:  # Only handle pygame events if render mode is enabled
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    running = False
        ...
        if not env.paused and not env.done:
            ...
            env.render()  # Only renders if render_mode is True
