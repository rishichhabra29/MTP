...
import time
import math
# Removed unused imports: folium, selenium, PIL
import os
import copy
...
class Ship:
    ...
    def update_pixel_position(self):
        self.x, self.y = self.env.map_manager._latlon_to_pixels(self.lat, self.lon)
    ...
    # Calculate approximate degrees per pixel based on zoom level
        zoom_scale = 360.0 / (256.0 * (2 ** self.env.map_manager.zoom))
        dpp_lat = zoom_scale
        dpp_lon = zoom_scale / math.cos(math.radians(self.lat))
    ...
    # 3. Try adjusted headings to avoid collisions
        for adjustment in range(0, max_angle_adjustment + angle_increment, angle_increment):
            ...
            # Convert positions to pixels for LoS-based collision check
                current_px = self.env.map_manager._latlon_to_pixels(current_lat, current_lon)
                candidate_px = self.env.map_manager._latlon_to_pixels(candidate_lat, candidate_lon)
                target_px = self.env.map_manager._latlon_to_pixels(target_lat, target_lon)
...
class Torpedo:
    ...
    def update_pixel_position(self):
        ...
        self.x, self.y = self.source.env.map_manager._latlon_to_pixels(self.lat, self.lon)
    ...
    def fire_torpedo(self, ship, target):
        ...
        if ship.torpedo_count > 0:
            ...
            torpedo = Torpedo(
                ...
                source=ship,
                target=target,
                map_manager=self.env.map_manager
            )
...
class DecoyMissileManager:
    ...
    def handle_DecoyM_firing_mechanics(self):
        if self.env.attacker_ship.torpedoes:
            for active_torpedo in self.env.attacker_ship.torpedoes:
                torpedo_px = self.env.map_manager._latlon_to_pixels(active_torpedo.lat, active_torpedo.lon)

                for defender in self.env.defender_ships:
                    if defender.ship_type == 'def_decoyM':
                        defender_px = self.env.map_manager._latlon_to_pixels(defender.lat, defender.lon)
    ...
    def fire_missile(self, ship, target):
        if ship.decoyM_count > 0:
            ...
            missile['x'], missile['y'] = self.env.map_manager._latlon_to_pixels(missile['lat'], missile['lon'])
    ...
    def update_decoy_missile(self):
        for ship in self.env.defender_ships:
            ...
            for missile in ship.decoy_missile:
                ...
                # Check for hit
                target_x, target_y = self.env.map_manager._latlon_to_pixels(target.lat, target.lon)
...
class HelicopterManager:
    ...
    def update_pixel_position(self):
        ...
        if self.helicopter_lat is not None and self.helicopter_lon is not None:
            self.helicopter_x, self.helicopter_y = self.env.map_manager._latlon_to_pixels(
                self.helicopter_lat,
                self.helicopter_lon
            )
...
class MapManager:
    """Manages the background map tiles for the Pygame renderer."""
    def __init__(self, screen_width: int, screen_height: int, font: pygame.font.Font):
        self.width = screen_width
        self.height = screen_height
        self.font = font
        self.tile_server_url = "https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}"
        self.tile_size = 256
        self.tile_cache = {}
        self.zoom = 5
        self.center_lon = 80.0
        self.center_lat = 5.0
        self.map_surface = pygame.Surface((self.width, self.height))
        self.update_map()

    def lonlat_to_tile_coords(self, lon: float, lat: float, zoom: int) -> Tuple[float, float]:
    ...
    def tile_coords_to_lonlat(self, xtile: float, ytile: float, zoom: int) -> Tuple[float, float]:
        n = 2.0 ** zoom
    ...
    def _fetch_tile(self, z: int, x: int, y: int) -> Optional[pygame.Surface]:
        tile_key = (z, x, y)
        ...
        try:
            ...
            tile_surface = pygame.image.load(image_data).convert()
            self.tile_cache[tile_key] = tile_surface
            return tile_surface
    ...
    def update_map(self):
        center_x_tile, center_y_tile = self.lonlat_to_tile_coords(self.center_lon, self.center_lat, self.zoom)
        offset_x = self.width / 2 - (center_x_tile - int(center_x_tile)) * self.tile_size
        offset_y = self.height / 2 - (center_y_tile - int(center_y_tile)) * self.tile_size
        start_x_tile = int(center_x_tile) - math.ceil(offset_x / self.tile_size)
        start_y_tile = int(center_y_tile) - math.ceil(offset_y / self.tile_size)
        end_x_tile = int(center_x_tile) + math.ceil((self.width - offset_x) / self.tile_size)
        end_y_tile = int(center_y_tile) + math.ceil((self.height - offset_y) / self.tile_size)
        for x in range(start_x_tile, end_x_tile):
            for y in range(start_y_tile, end_y_tile):
                tile_surface = self._fetch_tile(self.zoom, x, y)
                if tile_surface:
                    blit_pos_x = (x - start_x_tile) * self.tile_size + (offset_x % self.tile_size) - self.tile_size
                    blit_pos_y = (y - start_y_tile) * self.tile_size + (offset_y % self.tile_size) - self.tile_size
                    self.map_surface.blit(tile_surface, (blit_pos_x, blit_pos_y))
        self._draw_grid()

    def _draw_grid(self):
        grid_color = (255, 255, 255, 150)
        if self.zoom < 6: spacing = 10.0         # Degrees
        elif self.zoom < 8: spacing = 5.0        # Degrees
        elif self.zoom < 10: spacing = 1.0       # Degrees
        elif self.zoom == 10: spacing = 1/10     # 0.1 deg = 6 minutes
        elif self.zoom == 11: spacing = 1/60     # ~0.0167 deg = 1 minute
        elif self.zoom >= 12: spacing = 1/120    # ~0.0083 deg = 30 seconds
        center_x_tile, center_y_tile = self.lonlat_to_tile_coords(self.center_lon, self.center_lat, self.zoom)
        tl_x_tile = center_x_tile - (self.width / 2 / self.tile_size)
        tl_y_tile = center_y_tile - (self.height / 2 / self.tile_size)
        br_x_tile = center_x_tile + (self.width / 2 / self.tile_size)
        br_y_tile = center_y_tile + (self.height / 2 / self.tile_size)
        tl_lon, tl_lat = self.tile_coords_to_lonlat(tl_x_tile, tl_y_tile, self.zoom)
        br_lon, br_lat = self.tile_coords_to_lonlat(br_x_tile, br_y_tile, self.zoom)
        start_lon = math.floor(tl_lon / spacing) * spacing
        end_lon = math.ceil(br_lon / spacing) * spacing
        for lon in np.arange(start_lon, end_lon, spacing):
            points = []
            for lat_step in np.linspace(br_lat, tl_lat, 20):
                x_tile, y_tile = self.lonlat_to_tile_coords(lon, lat_step, self.zoom)
                screen_x = (x_tile - tl_x_tile) * self.tile_size
                screen_y = (y_tile - tl_y_tile) * self.tile_size
                points.append((screen_x, screen_y))
            if len(points) > 1:
                pygame.draw.aalines(self.map_surface, grid_color, False, points)
                if spacing >= 1.0:
                    label = self.font.render(f"{abs(lon):.1f}°{'W' if lon < 0 else 'E'}", True, (255,255,255))
                elif spacing >= 1/60:
                    # Label with degrees and minutes
                    deg = int(abs(lon))
                    minutes = int((abs(lon) - deg) * 60)
                    label = self.font.render(f"{deg}°{minutes:02d}'{'W' if lon < 0 else 'E'}", True, (255,255,255))
                else:
                    # Label with degrees, minutes, seconds
                    deg = int(abs(lon))
                    minutes = int((abs(lon) - deg) * 60)
                    seconds = int((((abs(lon) - deg) * 60) - minutes) * 60)
                    label = self.font.render(f"{deg}°{minutes:02d}'{seconds:02d}\"{'W' if lon < 0 else 'E'}", True, (255,255,255))
                self.map_surface.blit(label, (points[0][0] + 5, 10))
        start_lat = math.floor(br_lat / spacing) * spacing
        end_lat = math.ceil(tl_lat / spacing) * spacing
        for lat in np.arange(start_lat, end_lat, spacing):
            points = []
            for lon_step in np.linspace(tl_lon, br_lon, 20):
                x_tile, y_tile = self.lonlat_to_tile_coords(lon_step, lat, self.zoom)
                screen_x = (x_tile - tl_x_tile) * self.tile_size
                screen_y = (y_tile - tl_y_tile) * self.tile_size
                points.append((screen_x, screen_y))
            if len(points) > 1:
                pygame.draw.aalines(self.map_surface, grid_color, False, points)
                if spacing >= 1.0:
                    label = self.font.render(f"{abs(lat):.1f}°{'S' if lat < 0 else 'N'}", True, (255,255,255))
                elif spacing >= 1/60:
                    # Label with degrees and minutes
                    deg = int(abs(lat))
                    minutes = int((abs(lat) - deg) * 60)
                    label = self.font.render(f"{deg}°{minutes:02d}'{'W' if lat < 0 else 'E'}", True, (255,255,255))
                else:
                    # Label with degrees, minutes, seconds
                    deg = int(abs(lat))
                    minutes = int((abs(lat) - deg) * 60)
                    seconds = int((((abs(lat) - deg) * 60) - minutes) * 60)
                    label = self.font.render(f"{deg}°{minutes:02d}'{seconds:02d}\"{'W' if lat < 0 else 'E'}", True, (255,255,255))
                self.map_surface.blit(label, (10, points[0][1] + 5))




    def zoom_in(self):
        self.zoom = min(12, self.zoom + 1)
        self.update_map()
    ...
    def pan(self, direction, km_shift=5):
    
    #  Pans the map by shifting the center by km_shift kilometers in the given direction.
    #  direction: one of ['left', 'right', 'up', 'down']
    
    
        km_per_deg_lat = 110.574
        km_per_deg_lon = 111.320 * math.cos(math.radians(self.center_lat))

        lat_shift = 0
        lon_shift = 0
        if direction == "left":
            lon_shift = -km_shift / km_per_deg_lon
        elif direction == "right":
            lon_shift = km_shift / km_per_deg_lon
        elif direction == "up":
            lat_shift = km_shift / km_per_deg_lat
        elif direction == "down":
            lat_shift = -km_shift / km_per_deg_lat

        self.center_lat += lat_shift
        self.center_lon += lon_shift

        # Clamp latitude to [-90, 90]
        self.center_lat = max(-90, min(90, self.center_lat))
        # Wrap longitude within [-180, 180]
        if self.center_lon > 180:
            self.center_lon -= 360
        elif self.center_lon < -180:
            self.center_lon += 360
    ...
    def geo_to_pixel(self, lat, lon):
        """Convert lat/lon to screen pixel using the map manager's center and zoom"""
        x_tile_c, y_tile_c = self.lonlat_to_tile_coords(self.center_lon, self.center_lat, self.zoom)
        x_tile, y_tile = self.lonlat_to_tile_coords(lon, lat, self.zoom)
        dx = (x_tile - x_tile_c) * self.tile_size + self.width / 2
        dy = (y_tile - y_tile_c) * self.tile_size + self.height / 2
        return int(dx), int(dy)

    def pixel_to_geo(self, x, y):
        """Converts Pygame screen (x, y) to (lat, lon) using the MapManager's projection."""
        # Center tile coords
        center_x_tile, center_y_tile = self.lonlat_to_tile_coords(
            self.center_lon, self.center_lat, self.zoom
        )
        # Calculate tile delta from screen pixel
        delta_x_tiles = (x - self.width / 2) / self.tile_size
        delta_y_tiles = (y - self.height / 2) / self.tile_size
        # Tile coordinates at (x, y)
        tile_x = center_x_tile + delta_x_tiles
        tile_y = center_y_tile + delta_y_tiles
        # Convert tile coordinates to lat/lon
        lon, lat = self.tile_coords_to_lonlat(tile_x, tile_y, self.zoom)
        return lat, lon  # (lat, lon)

    def _latlon_to_pixels(self, lat, lon):
        """Convert lat/lon to screen pixel - compatibility wrapper for geo_to_pixel"""
        return self.geo_to_pixel(lat, lon)
    def _latlon_to_pixels(self, lat, lon):
        """Convert lat/lon to screen pixel - compatibility wrapper for geo_to_pixel"""
        return self.geo_to_pixel(lat, lon)
    ...
    def render_background(self):
        """Renders the real-world map as the background"""
        # The map is already updated in self.map_surface
        # No additional rendering needed here as we'll blit map_surface in render()
        pass


    def pixels_to_latlon(self, x, y):
        """Converts pixel (x, y) coordinates back to latitude & longitude - compatibility wrapper"""
        return self.pixel_to_geo(x, y)


    def _render_latlon(self, screen, x, y, rect_size):
        """
...
class Renderer:
    ...
    def _render_ship(self, ship, color=(255, 255, 255), base_rect_size=15, line_thickness=1):
        ...
        if ship.ship_health > 0:
            ...
            self.map_generator._render_latlon(self.env.screen, x, y, rect_size)
    ...
    def _render_helicopter(self, latlon):
        ...
        if self.env.helicopManager.helicopter_lat is not None:
            ...
            # Draw position label
            self.map_generator._render_latlon(self.env.screen, x, y, size * 2)
...
class NavalShipEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.map_manager = MapManager(self.width, self.height, self.font)
        # Set initial center and zoom from user input
        self.map_manager.center_lat = self.map_center[0]
        self.map_manager.center_lon = self.map_center[1]
        self.map_manager.zoom = self.zoom
        ...
        self.renderer = Renderer(self, self.map_manager)
        ...
        self.base_location_inPixels = self.map_manager._latlon_to_pixels(self.base_location[0], self.base_location[1])
    ...
    def _reload_map_image(self):
        # Update map manager center and zoom
        self.map_manager.center_lat = self.map_center[0]
        self.map_manager.center_lon = self.map_center[1]
        self.map_manager.zoom = self.zoom
        self.map_manager.update_map()

        self.base_location_inPixels = self.map_manager._latlon_to_pixels(
            ...
            if hasattr(ship, 'decoy_missile'):
                for missile in ship.decoy_missile:
                    if 'lat' in missile and 'lon' in missile:
                        missile['x'], missile['y'] = self.map_manager._latlon_to_pixels(missile['lat'], missile['lon'])
...
#         return
    ...
    def render(self, mode='human'):
        ...
        self.map_manager.update_map()
        self.screen.blit(self.map_manager.map_surface, (0, 0))  # Background map
